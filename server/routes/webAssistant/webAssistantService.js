...
Promise.resolve().then(() => {
  loader.load().then((docs) => {
    ...
    console.dir(filteredDocs);
    retriever.addDocuments(filteredDocs);
  });
});
...
const model = new ChatOpenAI({ modelName: "gpt-4", verbose: true });
...
async function processMessage(message) {
  try {
    retriever.getRelevantDocuments(message).then((res) => {
      console.dir({
        message,
        res,
      });
